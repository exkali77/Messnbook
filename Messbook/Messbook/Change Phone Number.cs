...
namespace Messbook
{
    public partial class Change_Phone_Number : Form
    {
        ...
        private void Mess_regihst()
        {
            ...
            {
                reader.Close();
                SqlCommand command1 = new SqlCommand("Update Messenbok_regihstirasyon set phone=@gm where username=@ad", connection);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (reader.Read())
            {
                if (textBox1.Text != "")
                {
                    if (Cryptolojy.Encryption(textBox1.Text, 3) != reader["phone"].ToString())
                    {
                        reader.Close();
                        connection.Close();
                        Mess_regihst();
                    ...
                    else
                    {
                        reader.Close();
                        connection.Close();
                        label2.Text = "An account already exists with this Phone";
                    }
                }
                else
                {
        ...
        private void Change_Phone_Number_Load(object sender, EventArgs e)
        {
